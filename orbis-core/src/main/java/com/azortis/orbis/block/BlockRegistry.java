 ...
 * internally using {@link PlatformBlockRegistry}. This class is also used for creating {@link com.azortis.orbis.block.entity.BlockEntity}'s
 *
 * @author Jake Nijssen
 * @since 0.3-Alpha
 */
...
public final class BlockRegistry {
    ...
    private static PlatformBlockRegistry PLATFORM_REGISTRY = null;

    private BlockRegistry() {
    }

    /**
     * Sets the internal {@link PlatformBlockRegistry} for this registry to delegate to.
     *
     * @param platformRegistry The platform implementation of the internal registry.
     * @since 0.3-Alpha
     */
    @API(status = API.Status.INTERNAL, since = "0.3-Alpha", consumers = {"com.azortis.orbis.paper"})
    public static void init(@NotNull PlatformBlockRegistry platformRegistry) {
        if (PLATFORM_REGISTRY == null) {
    ...
    public static @NotNull ImmutableSet<Key> blockKeys() {
        return PLATFORM_REGISTRY.blockKeys();
    }
    ...
    public static @NotNull ImmutableSet<Block> blocks() {
        return PLATFORM_REGISTRY.blocks();
    }
    ...
    public static @NotNull ImmutableSet<BlockState> states() {
        return PLATFORM_REGISTRY.states();
    }

    @Contract(pure = true)
    public static boolean containsKey(@NotNull Key key) {
        return PLATFORM_REGISTRY.containsKey(key);
    }

    @Contract(pure = true)
    public static @NotNull Block fromKey(@NotNull Key key) throws IllegalArgumentException {
        Preconditions.checkArgument(PLATFORM_REGISTRY.containsKey(key), "Invalid block key!");
        return PLATFORM_REGISTRY.fromKey(key);
    }
    ...
    public static boolean containsKey(@NotNull String key) throws IllegalArgumentException {
        ...
        return PLATFORM_REGISTRY.containsKey(Key.key(key));
    }
    ...
    public static @NotNull Block fromId(final int id) throws IllegalArgumentException {
        ...
        return PLATFORM_REGISTRY.fromId(id);
    }
    ...
    public static @NotNull BlockState fromStateId(final int stateId) throws IllegalArgumentException {
        ...
        return PLATFORM_REGISTRY.fromStateId(stateId);
    }
